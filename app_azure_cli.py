    ...
    with st.sidebar:
        ...
        if not st.session_state.initialized:
            ...
            if st.button("🔄 Force Token Refresh"):
                try:
                    new_token = st.session_state.token_manager.force_refresh_token()
                    if new_token:
                        st.success("✅ Token refreshed successfully!")
                        # Reinitialize MCP with new token
                        tools, agent = asyncio.run(st.session_state.mcp_client.initialize(new_token))
                        if tools and agent:
                            st.session_state.agent = agent
                            st.session_state.last_token_refresh = datetime.now()
                            st.success("✅ MCP reinitialized with new token!")
                        else:
                            st.error("❌ Failed to reinitialize MCP")
                    else:
                        st.error("❌ Failed to refresh token")
                except Exception as e:
